 ...
 * @returns {Promise<{data: any[], hasNext: boolean} | null>} 세션 목록과 다음 페이지 존재 여부를 반환합니다.
 */
const fetcher = async (page: number, statuses?: string[]): Promise<{ data: any[]; hasNext: boolean; } | null> => {
  const finalStatuses =
 ...
 * @returns {{sessions: any[], isLoading: boolean, error: Error | null, hasNextPage: boolean, loadMore: Function, refetch: Function}}
 */
export const useParticipatingSessionList = (): { sessions: any[]; isLoading: boolean; error: Error | null; hasNextPage: boolean; loadMore: Function; refetch: Function; } => {
